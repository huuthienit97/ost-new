...
export function setupSwagger(app: Express) {
  // Dynamic Swagger setup that works on both localhost and production
...
3. Test các API theo nhóm chức năng
    ...
    const options = {
      definition: {
        ...
        info: {
          title: 'CLB Sáng Tạo - API Documentation',
          version: '2.1.0',
          description: 'API documentation cho hệ thống quản lý câu lạc bộ sáng tạo',
        },
        servers: [{ url: baseUrl, description: 'Current Server' }],
        // ... add full schema here if needed
      },
      apis: [],
    };
  ...
  app.get('/postman-collection.json', (req, res) => {
    ...
    res.setHeader('Content-Type', 'application/json');
    res.setHeader('Content-Disposition', 'attachment; filename="CLB-SangTao-Production-API.postman_collection.json"');
    res.send(collection);
  });
