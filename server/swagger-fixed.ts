...
2. Thêm token vào Authorization header: Bearer <token>
              ...
              'application/json': {
                schema: {
                  ...
                  properties: {
                    ...
                    fullName: { type: 'string', example: 'Người dùng mới' },
                    password: { type: 'string', example: 'password123' },
...
export function setupFixedSwagger(app: Express) {
  // Serve swagger docs with consistent UI
  ...
  app.get('/postman-collection.json', (req, res) => {
    const collection = {
      info: {
        name: 'CLB Sáng Tạo API - Complete',
        description: 'Bộ sưu tập API đầy đủ cho hệ thống quản lý câu lạc bộ sáng tạo',
        schema: 'https://schema.getpostman.com/json/collection/v2.1.0/collection.json',
      },
      variable: [
        { key: 'baseUrl', value: 'http://localhost:5000' },
        { key: 'productionUrl', value: 'https://your-domain.com' },
        { key: 'authToken', value: '' },
        { key: 'apiKey', value: '' },
      ],
      auth: {
      ...
      item: [
        {
          ...
          item: [
            {
              name: 'Get Public Users with Position & Department',
              request: {
                method: 'GET',
                url: '{{baseUrl}}/api/public/users',
                description: 'Lấy danh sách người dùng với đầy đủ thông tin chức vụ và ban'
              }
            }
          ]
        },
    ...
    res.setHeader('Content-Type', 'application/json');
    res.setHeader('Content-Disposition', 'attachment; filename="CLB-SangTao-Complete-API.postman_collection.json"');
    res.send(collection);
  });
